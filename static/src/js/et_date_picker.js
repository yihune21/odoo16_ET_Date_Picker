...
class DateTimeEthiopiaField extends Component {
  ...
  syncWithOdooField() {
    ...
    if (odooFieldValue && (!this.state.selectedDate || this.state.selectedDate.toGregorian().toISOString() !== odooFieldValue)) {
        // Parse the date string
        const [day, month, year] = odooFieldValue.split('/');
        const gregorianDate = new EthiopianDate(year,month,day).toGregorian()
        // Check if the gregorianDate is valid
        if (!isNaN(gregorianDate.getTime())) {
            try {
                const ethiopianDate = EthiopianDate.fromGregorian(gregorianDate);
                // Check if ethiopianDate is valid and has the required properties
    ...
    onDateSelect=(day) => {
       const selectedDate = new EthiopianDate(this.state.currentYear, this.state.currentMonth + 1, day);
        ...
        // Update the Ethiopian date field
        this.props.update(formattedEthiopianDate);

        // Trigger onchange to update Odoo field
