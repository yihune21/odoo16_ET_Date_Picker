...
class PurchaseOrder(models.Model):
    ...
    purchase_date_et = fields.Char(string='Purchase Date (ET)', compute='_compute_et_date',store=True)
        ...
        for order in self:
            if order.date_order:
                try:
                    ...
                    order.purchase_date_et = f"{et_date_tuple[2]:02d}/{et_date_tuple[1]:02d}/{et_date_tuple[0]}"
                except Exception:
